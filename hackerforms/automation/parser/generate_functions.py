...
from .templates.widget_schema_template import widget_schema_template
from .helpers import (
    get_default_key,
...
from .metadata_parser import load_widget_metadata
from .docs_parser_old import group_docs_by_section, generate_page_method_docs
...
def extract_widgets_methods(types, postfix="Input"):
    ...
    widget_types = filter_classes(classes, postfix)
    ...
    for widget, widget_obj in widgets.items():
        ...
        widgets[widget]["func_name"] = get_function_name(widget, postfix=postfix)
...
def generate_input():
    ...
    input_functions = Template(read_functions_template).render(widgets=input_widgets)
    save("inputs", input_functions)


def generate_output():
    output_widgets = extract_widgets_methods(output_types, postfix="Output")
    output_functions = Template(display_functions_template).render(
...
def generate_page():
    input_widgets = extract_widgets_methods(input_types)
    output_widgets = extract_widgets_methods(output_types, postfix="Output")
    widget_schema = Template(widget_schema_template).render(
        input_widgets=input_widgets, output_widgets=output_widgets
    )
    save("widget_schema", widget_schema)


def generate_metadata_dict():
    metadata = load_widget_metadata()
    save("metadata", f"metadata = {str(metadata)}")
