...
POSITIONAL_ARGS_SECTION = 'Positional Arg(s):'
KEYWORD_ARGS_SECTION = 'Keyword Arg(s):'

def group_docs_by_section(docs: str) -> Dict[str, str]:  
  description, positional_arguments, keyword_arguments = get_docs_sections(parse_docs(docs))
...
def parse_docs(docs: str) -> List[str]:
  try:
    ...
    raise Exception("Add widget documentation on widget class constructor __init__.")
...
def get_docs_sections(docs: List[str]):
  return get_description_from_docs(docs),  get_position_args_from_docs(docs), get_keyword_args_from_docs(docs)
...
def get_description_from_docs(docs: List[str]) -> List[str]:
  ...
  if current == 0:
    raise Exception("Description not found. Please, write docs with description, positional args and keyword args.")

  if current == len(docs):
    raise Exception("Positional arguments not found. Please, write docs with description, positional args and keyword args.")
...
def get_position_args_from_docs(docs: List[str]) -> List[str]:
  ...
  while current < len(docs):
    if positional_not_found and not docs[current].startswith(POSITIONAL_ARGS_SECTION):
      current += 1
      continue
...
def get_keyword_args_from_docs(docs: List[str]) -> List[str]:
  ...
  if len(keyword_args) == 0:
    raise Exception("Keyword args not found. Please, describe the possible keyword arguments in widget type method.")

  return keyword_args


def generate_page_method_docs(docs: Dict, key_default: Optional[str] = None, widget_type: str = 'input') -> Dict:
  
  keyword_arguments = process_keywords(docs['keyword_arguments'], key_default, widget_type, )
...
def process_keywords(keyword_arguments: List[str], key_default: Optional[str] = None, widget_type: str = 'input') -> List[str]:
...
def remove_button_text(keyword_arguments: List[str]) -> List[str]:
  return list(filter(lambda argument: not argument.startswith('button_text'), keyword_arguments))
